...
void compareWeekSubstitutions(
    WeekSubstitutions actual, WeekSubstitutions target) {
  expect(actual.weekSubstitutions.length, target.weekSubstitutions.length,
      reason: "Week substitutions have different lengths");
  for (final daySubstitutionsEntry in target.weekSubstitutions.entries) {
    expect(
        actual.weekSubstitutions.containsKey(daySubstitutionsEntry.key), true,
        reason:
            "Key ${daySubstitutionsEntry.key} was expected, but is not present");
    final actualDaySubstitutions =
        actual.weekSubstitutions[daySubstitutionsEntry.key]!;
    expect(actualDaySubstitutions.item2, daySubstitutionsEntry.value.item2,
        reason: "Dates do not match (wtf ?)");
    expect(actualDaySubstitutions.item1.length,
        daySubstitutionsEntry.value.item1.length,
        reason:
            "Substitution count does not match on day ${daySubstitutionsEntry.key}");
    for (var i = 0; i < daySubstitutionsEntry.value.item1.length; i++) {
      ...
      expect(actualSubstitution.item2, targetSubstitution.item2,
          reason:
              "Integration names do not match; day: ${daySubstitutionsEntry.key} index: $i");
      expect(actualSubstitution.item1, targetSubstitution.item1,
          reason:
              "Substitution data does not match; day: ${daySubstitutionsEntry.key} index: $i");
    }
  }
}
...
void main() {
  ...
  test("Merge test same day", () {
    ...
    final weekSubstitutionsTarget = WeekSubstitutions(null, "");
    weekSubstitutionsTarget.weekSubstitutions[substDate.weekday.toString()] =
        Tuple2([
  ...
  test(
      "Merge test no change single range and multi range without overwrite equal",
      () {
    final weekSubstitutionsA = WeekSubstitutions(null, "A");
    final substDate = DateTime.parse("2022-03-23");
    weekSubstitutionsA.setDay([
  ...
  test("Merge test irrelevant properties without overwrite equal", () {
    ...
    final substDate = DateTime.parse("2022-03-23");
    weekSubstitutionsA.setDay([
      {"Stunde": "1-2", "Art": "Entfall"},
      {"Stunde": "5-6", "(Le.) nach": "Hosp"},
      {"Stunde": "8"}
    ], substDate, "A");
    final weekSubstitutionsB = WeekSubstitutions(null, "B");
    weekSubstitutionsB.setDay([
  ...
  test("Test remove overlaying", () {
    ...
    expect(weekSubstitutions.removeOverlaying(), true,
        reason: "Expected overlaps");
  ...
  test("Test remove overlaying no overlaying", () {
    ...
    expect(weekSubstitutions.removeOverlaying(), false,
        reason: "Expected no removing");
