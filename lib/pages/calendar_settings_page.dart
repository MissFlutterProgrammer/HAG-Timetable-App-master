...
class _CalendarSettingsPageState extends State<CalendarSettingsPage> {
  ...
  void initState() {
    ...
    for (final calendarType in pluginCalendarTypes) {
      final currentUrl =
          sharedState.profileManager.calendarUrls[calendarType.name()];
      if (currentUrl != null) {
  ...
  Future<void> saveAndGoBack() async {
    for (var i = 0; i < urlList.length; i++) {
      ...
      final urlRegex = RegExp(
          r"^https:\/\/hag-iserv.de\/(caldav\/.+\/calendar|iserv\/public\/calendar\/ics\/feed\/plugin\/[a-z0-9]{128}\/calendar.ics)$");
