class Holiday {
  ...
  Holiday({int? month, int? day, this.daysAfterEaster}) {
...
DateTime getEasterDate(int year) {
  ...
  final double centuryNumber =
      (15 - magicNumber + century - (century / 4)) % 30;
  final double diffLeapYearsJulianGergorian = (4 + century - (century / 4)) % 7;
  final double numberOfDaysToSpringStart =
      (19 * locationInMetonicCyle + centuryNumber) % 30;
  final double numberOfDaysToNextSunday = (diffLeapYearsJulianGergorian +
          2 * numberOfLeapYears +
          4 * nonLeapYear +
          6 * numberOfDaysToSpringStart) %
      7;

  if (numberOfDaysToSpringStart == 29 && numberOfDaysToNextSunday == 6) {
    return DateTime(year, 04, 19);
  }
  if (numberOfDaysToSpringStart == 28 && numberOfDaysToNextSunday == 6) {
    return DateTime(year, 04, 18);
  }
  final int days =
      (22 + numberOfDaysToSpringStart + numberOfDaysToNextSunday).floor();
