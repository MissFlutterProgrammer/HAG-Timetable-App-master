// ignore_for_file: depend_on_referenced_packages, unreachable_from_main
...
const String notifyDebugLogFileLocation =
    "/storage/emulated/0/Android/data/stundenplan-notfiy-debug.log";
...
NotificationDetails getNotificationDetails() {
  const AndroidNotificationDetails androidPlatformChannelSpecifics =
      AndroidNotificationDetails(
          'timetable_notifications', 'Stundenplan Benachrichtigungen',
          channelDescription:
              'Wenn Änderungen im Stunden- oder Vertretungsplan vorhanden sind, gibt es eine Benachrichtigung',
...
Tuple2<String, String>? getSubstitutionsNotificationText(
    Map<String, dynamic> substitutionsBeforeJson,
  ...
  final weekSubstitutionsBefore1 = WeekSubstitutions(
      substitutionsBeforeJson, "before",
      checkWeekDay: false, checkWeek: false);
  if (weekSubstitutionsBefore1.removeOverlaying()) {
    fileLog("Found bad overlaying substitutions in old substitutions",
        name: "getSubstitutionsNotificationText");
  }
  final weekSubstitutions = WeekSubstitutions(substitutionsJson, "now",
      checkWeekDay: false, checkWeek: false);
  if (weekSubstitutions.removeOverlaying()) {
    fileLog("Found bad overlaying substitutions in new substitutions",
        name: "getSubstitutionsNotificationText");
  }
  weekSubstitutionsBefore1.merge(weekSubstitutions, "now");
  for (final substitution
      in weekSubstitutionsBefore1.weekSubstitutions.entries) {
    final weekDay = DateTime.parse(substitution.value.item2).weekday;
    final daySubstitutions = substitution.value.item1;
    changes.addAll(daySubstitutions
        .where((e) => e.item2 == "now")
  ...
  final weekSubstitutionsBefore2 = WeekSubstitutions(
      substitutionsBeforeJson, "before",
      checkWeekDay: false, checkWeek: false)
    ..removeOverlaying();
  weekSubstitutionsBefore2.merge(weekSubstitutions, "now",
      overwriteEqual: true);
  for (final substitution
      in weekSubstitutionsBefore2.weekSubstitutions.entries) {
    final weekDay = DateTime.parse(substitution.value.item2).weekday;
    final daySubstitutions = substitution.value.item1;
    changes.addAll(daySubstitutions
        .where((e) => e.item2 == "before")
  ...
  for (final change in changes) {
    ...
    final anchorText = originalSubject ??
        substitution[
            "Stunde"]; // Text that specifies what lesson or time frame the change is targeting (aka something, where the user can directly infer, when the change is happening)
    final substitutionTextMessage =
        customStrip(substitution["Text"] as String? ?? "")
                    .replaceAll("\u{00A0}", "")
                    .isNotEmpty &&
                substitution["Text"] != "---" &&
                !revertedChange
    ...
    if (isDropped == "x") {
      changeTexts.add(Tuple3(
          ...
          revertedChange ? 0 : 1));
    } else if (substitution["Fach"] != substitution["statt Fach"] &&
        substitution.containsKey("Fach") &&
        substitution.containsKey("statt Fach")) {
      ...
      changeTexts.add(Tuple3(
          ...
          revertedChange ? 2 : 3));
    } else if (substitution["Raum"] != substitution["statt Raum"] &&
        substitution.containsKey("Raum") &&
        substitution.containsKey("statt Raum")) {
      ...
      changeTexts.add(Tuple3(
          ...
          revertedChange ? 4 : 5));
    } else if (substitution["Vertretung"] != substitution["statt Lehrer"] &&
        substitution.containsKey("Vertretung") &&
        substitution.containsKey("statt Lehrer")) {
...
void cleanupWeekSubstitutionJson(
    Map<String, dynamic> substitutionsJson, List<String> allSubjects) {
  final now = clock.now();
  ...
  substitutionsJson.removeWhere((key, value) {
    ...
    return !((date.isAfter(weekStartDate) ||
            date.isAtSameMomentAs(weekStartDate)) &&
  ...
  substitutionsJson.forEach((key, value) =>
      ((value as List<dynamic>)[0] as List<dynamic>)
          .removeWhere((substitution) {
        final substitutionMap = substitution as Map<String, dynamic>;
        // Strip properties
        substitutionMap["Stunde"] =
            customStrip(substitutionMap["Stunde"] as String);
        substitutionMap["Fach"] =
            customStrip(substitutionMap["Fach"] as String);
        substitutionMap["Raum"] =
            customStrip(substitutionMap["Raum"] as String);
        substitutionMap["statt Raum"] =
            customStrip(substitutionMap["statt Raum"] as String);
        substitutionMap["statt Lehrer"] =
            customStrip(substitutionMap["statt Lehrer"] as String);
        substitutionMap["Vertretung"] =
            customStrip(substitutionMap["Vertretung"] as String);
        substitutionMap["Raum"] =
            customStrip(substitutionMap["Raum"] as String);
        substitutionMap["Entfall"] =
            customStrip(substitutionMap["Entfall"] as String);
        // Remove subjects that the user dose not have
        final originalSubject =
            customStrip(substitutionMap["statt Fach"] as String);
        substitutionMap["statt Fach"] = originalSubject;
        return originalSubject != "\u{00A0}" &&
            !allSubjects.contains(originalSubject);
      }));
}
...
void callbackDispatcher() {
  Workmanager().executeTask((task, inputData) async {
    ...
    if (currentHour >= Constants.notifyUpdateDaySleepStartHour ||
        currentHour <= Constants.notifyUpdateDaySleepEndHour) {
      return true;
    }
    // Initialize plugins
    ...
    final sharedState = SharedState(
        preferences, Content(Constants.width, Constants.defaultHeight));
    if (sharedState.loadStateAndCheckIfFirstTime(fromBackgroundTask: true)) {
      return false; // Should not happen
    }
    if (!sharedState.sendNotifications) {
      return true; // Notifications are turned off
    }
    final allSubjects = sharedState.allCurrentSubjects;
    // Load previous substitutions and content from cache
    (Integrations.instance.getIntegrationByName("IServ")!
            as IServUnitsSubstitutionIntegration)
        .loadCheckWeekDay = false;
    (Integrations.instance.getIntegrationByName("Schulmanager")!
            as SchulmanagerIntegration)
        .loadCheckWeekDay = false;
    sharedState.loadCache();
    final substitutionsBefore =
    ...
    if (!const DeepCollectionEquality()
        .equals(substitutionsBeforeJson, substitutionsJson)) {
      sharedState.content.updateLastUpdated();
      sharedState.saveCache();
      fileLog(
          "substitutions before clean: $substitutionsBeforeJson\nparsed substitutions clean: $substitutionsJson\nsubstitutions before raw: ${substitutionsBefore.weekSubstitutions}\r\nparsed substitutions raw: ${substitutions.weekSubstitutions}",
          name: "start");
      final notificationText = getSubstitutionsNotificationText(
          substitutionsBeforeJson, substitutionsJson);
      if (notificationText != null) {
        fileLog(
            "Show notification: title: ${notificationText.item1}, body: ${notificationText.item2}",
            name: "end");
        await notifyPlugin.show(
            math.Random().nextInt(2147483647),
            notificationText.item1,
            notificationText.item2,
            platformChannelSpecifics);
        return true;
      } else {
        fileLog("Initial change detected, but no notification text",
            name: "end");
      }
    } else {
      fileLog("No change detected", name: "end");
    }
    if (!const DeepCollectionEquality()
        .equals(contentBeforeJson, sharedState.content.toJsonData())) {
      sharedState.content.updateLastUpdated();
      sharedState.saveCache();
...
Future<void> startNotificationTask() async {
  ...
  await Workmanager().registerPeriodicTask(
    ...
    constraints: Constraints(
        networkType: NetworkType.not_roaming, requiresBatteryNotLow: true),
  );
}
