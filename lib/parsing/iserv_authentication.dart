...
enum IServLoginResponseKind { ok, badPassword, badUsername, error }
...
Future<Tuple4<Uri, int, HttpClientResponse, String>> followRedirects(
    HttpClient client, CookieJar cj, HttpClientResponse response) async {
  Uri? uri;
  int redirects = 0;
  String? lastBody;
  var lastResponse = response;
  while (lastResponse.statusCode == 302 ||
      lastResponse.statusCode == 301 && redirects < 10) {
    // Get the new uri based on the last response
    final newLocation = lastResponse.headers.value("location")!;
    final newUri = newLocation.startsWith("/")
        ? Uri.parse("${Constants.iServHost}$newLocation")
...
Future<Tuple4<Uri, int, HttpClientResponse, String>> makeRequestWithRedirects(
    String method, Uri uri, HttpClient client, CookieJar cj,
  ...
  if (response.statusCode != 302) {
    ...
    final firstRequestBody =
        await response.transform(utf8.decoder).firstWhere((e) => true);
...
IServLoginResponseKind getIServLoginResponseKind(
...
Future<Tuple2<List<Cookie>?, IServLoginResponseKind>> getIServLoginCookies(
    Tuple2<String, String> iServCredentials) async {
  final cj = CookieJar();
  final client = HttpClient();
  // Make get request to login url to get redirected to the correct website
  final tempResult = await makeRequestWithRedirects(
      "GET", Uri.parse(Constants.loginUrlIServ), client, cj);
  await Future.delayed(const Duration(milliseconds: 300));
  // Send a post request to the login data
  final bodyString = getIServLoginPostBody(iServCredentials);
  final result = await makeRequestWithRedirects(
      "POST", tempResult.item1, client, cj,
      headers: iServLoginExtraHeaders, body: bodyString);
  final response = result.item3;
  final responseBody = result.item4;
  client.close();
  // Check for errors
  final responseKind =
      getIServLoginResponseKind(response.statusCode, responseBody);
...
Future<bool> areCookiesGood(String? cookies) async {
  ...
  final request = await client.openUrl(
      "GET", Uri.parse("${Constants.iServHost}/iserv/app/navigation/badges"));
  request.followRedirects = false;
  request.headers.add("Cookie", cookies);
  final HttpClientResponse response = await request.close();
  if (response.statusCode != 302) {
    response.transform(utf8.decoder).listen((_) {});
  }
...
Future<String?> iServLogin() async {
  ...
  for (final cookie in cookies) {
    if (cookie.name.startsWith("IServSAT") ||
        cookie.name.startsWith("IServSession")) {
