...
List<String> getRelevantSchoolClasses(SharedState sharedState) {
  ...
  if (!Constants.displayFullHeightSchoolGrades
      ...
      (sharedState.processSpecialCourseClass ||
          sharedState.hasChangedCourses)) {
    classes.add("${sharedState.profileManager.schoolGrade}K");
  }
  if ((sharedState.hasChangedCourses || sharedState.processSpecialAGClass) &&
      Constants.useAGs) {
    classes.add(Constants.specialClassNameAG);
  }
  return classes;
}
...
Future<void> parsePlans(SharedState sharedState) async {
  ...
  for (final schoolClassName in schoolClasses) {
    ...
    final getTimeTablesResult = await getTimeTableTables(
        schoolClassName, Constants.timeTableLinkBase, client);
    ...
    if (schoolClassName == "${sharedState.profileManager.schoolGrade}K") {
      sharedState.processSpecialCourseClass = true;
    }
    // Don't know why but for some reason main tables gets modified by getAvailableSubjectNames so we need to clone it.
    final classAvailableSubjects =
        getAvailableSubjectNames(timeTables.map((e) => e.clone(true)).toList());
  ...
  for (int i = 0; i < sharedState.profileManager.subjects.length; i++) {
    ...
    final possibleSubjects = mainAvailableSubjects
        .where((element) => element.toLowerCase() == subject.toLowerCase());
    ...
    if (possibleSubjects.length <= 1) {
      ...
      sharedState.profileManager.subjects[i] =
          findClosestStringInList(possibleSubjectList, subject);
    }
  }
  ...
  if (sharedState.hasChangedCourses) {
    ...
    if (agAvailableClassSubjects != null &&
        sharedState.profileManager.subjects.any((subject) =>
            agAvailableClassSubjects?.contains(subject) ?? false)) {
  ...
  for (var i = 0; i < classesTimeTables.length; i++) {
    ...
    await fillTimeTable(schoolClassName, classTimeTables, classTimeTableContent,
  ...
  await overwriteContentWithSubstitutionPlan(
          sharedState, client, content, sharedState.allCurrentSubjects)
      .timeout(Constants.clientTimeout);
  sharedState.content = content;
}
