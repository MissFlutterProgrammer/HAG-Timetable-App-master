// ignore_for_file: non_constant_identifier_names, no_leading_underscores_for_local_identifiers, avoid_dynamic_calls
...
Future<void> overwriteContentWithSubstitutionPlan(SharedState sharedState,
    Client client, Content content, List<String> subjects) async {
  try {
    await Integrations.instance.update(values: ["substitutions"]);
    final weekSubstitutions =
        Integrations.instance.getValue("substitutions") as WeekSubstitutions?;
    if (weekSubstitutions == null) {
      log("No substitutions available", name: "substitution");
      return;
    }
    ...
    for (final weekDayString in weekSubstitutions.weekSubstitutions.keys) {
      ...
      final daySubstitution =
          weekSubstitutions.weekSubstitutions[weekDayString];
      if (daySubstitution != null) {
        writeSubstitutionPlan(
            daySubstitution.item1, weekDay, content, subjects);
      }
    }
  } catch (e) {
    log("Error in overwriteContentWithSubstitutionPlan",
        name: "substitution", error: e);
  }
}

void writeSubstitutionPlan(List<Tuple2<Map<String, dynamic>, String>> plan,
    int weekDay, Content content, List<String> subjects) {
  try {
    for (var i = 0; i < plan.length; i++) {
      ...
      if (substitution.containsKey("Art")) {
        cell.substitutionKind = substitution["Art"]?.toString() ?? "";
      }
    ...
    log("Error in writeSubstitutionPlan", name: "substitution", error: e);
  }
}

class IServUnitsSubstitutionIntegration extends Integration {
  static const List<String> customSubstitutionProperties = [
    "(Le.) nach",
    "Art"
  ];
  ...
  void loadValuesFromJson(Map<String, dynamic> jsonValues) {
    for (final jsonValueEntry in jsonValues.entries) {
      if (jsonValueEntry.key == "substitutions") {
        values[jsonValueEntry.key] = WeekSubstitutions(
            jsonValueEntry.value as Map<String, dynamic>?, name,
            checkWeekDay: loadCheckWeekDay);
      }
    }
  }
  ...
  Future<Map<String, dynamic>> getCourseSubstitutionPlan(String course) async {
    final response = await client
        .get(Uri.parse('${Constants.substitutionLinkBase}_$course.htm'));
    if (Constants.defineHasTesterFeature) {
      lastResponses[course] = "${response.statusCode}\n\n${response.body}";
    }
    if (response.statusCode != 200) {
      log("Could not get substitution plan status code: ${response.statusCode}",
          name: "iserv-units-integration");
      return {
    ...
    if (document.outerHtml.contains("Fatal error")) {
      log("Could not parse substitution plan: Fatal error",
          name: "iserv-units-integration");
      return {
        "substitutions": <Map<String, String>>[],
        "substituteDate": DateTime.now(),
    ...
    if (!(document.body?.text.contains(course) ?? false)) {
      log("Could not load substitution for course: $course. Site does not contain course name",
          level: 3);
      return {
        "substitutions": <Map<String, String>>[],
        "substituteDate": DateTime.now(),
    ...
    for (var i = 0; i < headerRow.children.length; i++) {
      ...
      final headerName = baseHeaderInformation.firstWhereOrNull((headerName) {
        final checkHeaderName =
            headerName == "Lehrer" ? "(Lehrer)" : headerName;
    ...
    for (final row in rows) {
      ...
      for (var i = 0; i < headerInformation.length; i++) {
        ...
        substitutionKey =
            substitutionKey == "Lehrer" ? "Vertretung" : substitutionKey;
        String? beforeSubstitutionKey;
        if (["Fach", "Raum", "Vertretung"].contains(substitutionKey)) {
          beforeSubstitutionKey =
              "statt ${substitutionKey == "Vertretung" ? "Lehrer" : substitutionKey}";
        }
        ...
        if (columns[columnIndex].text == "\u{00A0}") {
          ...
          if (beforeSubstitutionKey != null) {
            substitution[beforeSubstitutionKey] = "\u{00A0}";
          }
        }
        ...
        if (strikethroughElements.isNotEmpty && beforeSubstitutionKey != null) {
          ...
          substitution[beforeSubstitutionKey] =
              beforeSubstitutionValue.replaceAll("\n", " ");
      ...
      if ((substitution["Art"] ?? substitution["(Le.) nach"] ?? "")
          .contains("Entfall")) {
...
class SchulmanagerIntegration extends Integration {
  ...
  Future<void> update() async {
    if (!active) return;
    if (sharedState.profileManager.schoolClassFullName !=
        sharedState.schulmanagerClassName) {
      return; // It does not make sense to show substitutions from the wrong class
    }
    final weekSubstitutions = values["substitutions"]! as WeekSubstitutions;
    ...
    for (final l in lessons as List<dynamic>) {
      ...
      final classHour = int.parse(
          (lesson["classHour"]! as Map<String, dynamic>)["number"]! as String);
      ...
      if (actualLesson == null && lesson.containsKey("originalLessons")) {
        ...
        final originalLessonTeacher =
            ((originalLesson["teachers"] as List<dynamic>?)?[0]
                as Map<String, dynamic>?)?["abbreviation"] as String?;
        final originalLessonRoom = (originalLesson["room"]
            as Map<String, dynamic>?)?["name"] as String?;
        if (!sharedState.allCurrentSubjects.contains(originalLessonSubject)) {
          continue;
        }
      ...
      if (!actualLesson.containsKey("room") || actualLesson["room"] == null) {
        continue;
      }
      final room =
          (actualLesson["room"] as Map<String, dynamic>)["name"]! as String;
      final teachers = actualLesson["teachers"]! as List<dynamic>;
      final teacher =
          (teachers[0] as Map<String, dynamic>)["abbreviation"] as String;
      final text = actualLesson["comment"] as String?;

      // Skip if nothing changed (aka not a substitution just redundant information)
      if (currentCell.subject == subject &&
          currentCell.teacher == teacher &&
          currentCell.room == room &&
          text == null) {
        continue;
      }
      final substitutionData = <String, dynamic>{
    ...
    for (final entry in substitutions.entries) {
      ...
      daySubstitutions.sort((a, b) => int.parse(a["Stunde"]! as String)
          .compareTo(int.parse(b["Stunde"]! as String)));
  ...
  Future<String?> getBundleVersion() async {
    ...
    final Response response =
        await client.get(Uri.parse("${Constants.schulmanagerBaseUrl}/#/login"));
  ...
  Future<void> setSchulmanagerClassName() async {
    if (sharedState.schulmanagerClassName == null) {
      ...
      final termResponse = await sendSchulmanagerApiRequest([
        {"endpointName": "get-current-term", "moduleName": null}
      ]);
      if (termResponse == null) {
        log("Could not get term id", name: "schulmanager-integration");
        return;
      }
      ...
      if (classesResponse == null) {
        log("Could not class ids with term id",
            name: "schulmanager-integration");
        return;
      }
      final classesData = classesResponse[0] as List<dynamic>;
      final schulmanagerClassData = classesData.firstWhereOrNull((classData) =>
          (classData as Map<String, dynamic>)["id"] == studentData["classId"]);
      if (schulmanagerClassData == null) {
        log("Could not find the class name, that is associated with the students class id",
            name: "schulmanager-integration");
        return;
      }
      // Save the class name
      sharedState.schulmanagerClassName =
          (schulmanagerClassData as Map<String, dynamic>)["name"] as String;
  ...
  Future<bool> setStudentDateFromJWT(String jwt) async {
    final userInfoResponse = await client.post(
        Uri.parse("${Constants.schulmanagerApiBaseUrl}/login-status"),
        headers: {"Authorization": "Bearer $jwt"});
    if (userInfoResponse.statusCode != 200) {
      log("Oauth failed on getting user info: ${userInfoResponse.statusCode} ${userInfoResponse.body}",
          name: "schulmanager-integration");
      return false;
    }
  ...
  Future<bool> loginOauth() async {
    ...
    final oidcRequest = Request(
        "Get",
        Uri.parse(
            "${Constants.schulmanagerOicdBaseUrl}/${Constants.schulmanagerSchoolId}"))
      ..followRedirects = false;
    final oidcResponse = await client.send(oidcRequest);
    if (oidcResponse.statusCode != 302) {
      log("Initializing Oauth failed: ${oidcResponse.statusCode}",
          name: "schulmanager-integration");
      return false;
    }
    final oidcCookieString = getCookieStringFromSetCookieHeader(
        oidcResponse.headers["set-cookie"]!, ["session", "session.sig"]);
    ...
    final iservAuthRequest = Request("Get", iservRedirectUri)
      ..followRedirects = false;
    ...
    if (iservAuthResponse.statusCode != 302) {
      log("Oauth failed on iserv login: ${iservAuthResponse.statusCode}",
          name: "schulmanager-integration");
      return false;
    }
    // Make callback request to get schulmanager session cookies
    final callbackRedirectUri =
        Uri.parse(iservAuthResponse.headers['location']!);
    final callbackRequest = Request("Get", callbackRedirectUri)
      ..followRedirects = false;
    ...
    if (callbackResponse.statusCode != 302) {
      log("Oauth failed on schulmanager callback: ${callbackResponse.statusCode}",
          name: "schulmanager-integration");
      return false;
    }
    final callBackSessionCookieString = getCookieStringFromSetCookieHeader(
        callbackResponse.headers["set-cookie"]!, ["session", "session.sig"]);
    // Get jwt
    final jwtResponse = await client.get(
        Uri.parse("${Constants.schulmanagerOicdBaseUrl}/get-jwt"),
        headers: {"Cookie": callBackSessionCookieString});
    if (jwtResponse.statusCode != 204) {
      log("Oauth failed on getting jwt: ${jwtResponse.statusCode} ${jwtResponse.body}",
          name: "schulmanager-integration");
      return false;
    }
  ...
  Future<List<dynamic>?> sendSchulmanagerApiRequest(
      List<Map<String, dynamic>> requests) async {
    final response = await client.post(
      Uri.parse("${Constants.schulmanagerApiBaseUrl}/calls"),
    ...
    if (response.statusCode != 200) {
      log("Api call error: ${response.statusCode} ${response.body}",
          name: "schulmanager-integration");
      return null;
    }
