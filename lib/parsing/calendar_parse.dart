...
Future<CalendarData> loadCalendarData(SharedState sharedState) async {
  ...
  final client = Client();
  for (final calenderUrlEntry
      in sharedState.profileManager.calendarUrls.entries) {
    final calenderType =
        CalendarTypeExtension.fromString(calenderUrlEntry.key)!;
    await getPluginCalendarData(
        calenderUrlEntry.value, calenderType, client, calendarData);
  }
  await getCaldavCalendarData("${Constants.calDavBaseUrl}/+public/calendar",
      CalendarType.public, client, calendarData);
  await getCaldavCalendarData(
      "${Constants.calDavBaseUrl}/klasse.${sharedState.profileManager.schoolClassFullName.toLowerCase()}/calendar",
      CalendarType.public,
      client,
      calendarData);
  {
    ...
    if (iServCredentials != null) {
      await getCaldavCalendarData(
          "${Constants.calDavBaseUrl}/schuelerinnen/calendar",
      ...
      //                                                              why ?
      await getCaldavCalendarData(
          "${Constants.calDavBaseUrl}/${iServCredentials.item1}/home",
...
Future<Map<String, String>?> enableAllCalendarPluginUrls() async {
  ...
  final response = await client.get(
      Uri.parse("${Constants.calendarIServBaseUrl}/plugin"),
      headers: getAuthHeaderFromCookies(cookies));
  ...
  for (final pluginRow in pluginRows) {
    ...
    final calendarPluginUrl =
        await enableCalendarPluginUrl(client, cookies, feedId);
    if (calendarPluginUrl == null) continue;
    // Get correct calendar type string from plugin name
    final calendarTypeString = pluginCalendarTypes
        .where((calendarType) => pluginName
            .toLowerCase()
            .contains(calendarType.name().toLowerCase()))
        .first
        .name();
...
Future<String?> enableCalendarPluginUrl(
    Client client, String cookies, String feedId) async {
  // Get plugin feed edit page
  var response = await client.get(
      Uri.parse("${Constants.calendarIServBaseUrl}/plugin/edit/$feedId"),
      headers: getAuthHeaderFromCookies(cookies));
  if (response.statusCode != 200) return null;
  ...
  response = await client.post(
      Uri.parse(
          "${Constants.calendarIServBaseUrl}/ics/feed/$feedId/generate/plugin?type=plugin&_token=$dataCsrfToken"),
      headers: getAuthHeaderFromCookies(cookies));
  if (response.statusCode != 200) {
    log("Cannot generate plugin toke. Error code ${response.statusCode}",
...
Future<CalendarData> getPluginCalendarData(String url, CalendarType type,
    Client client, CalendarData calendarData) async {
  final response = await client.get(Uri.parse(url));
  if (response.statusCode != 200) {
    log("Cannot get plugin calendar data with url $url. Error code ${response.statusCode}",
        name: "parsing.calendar");
    return calendarData;
  }
...
Future<CalendarData> getCaldavCalendarData(String url, CalendarType type,
    Client client, CalendarData calendarData) async {
  final iservCredentials = await getIServCredentials();
  if (iservCredentials == null) return calendarData;
  final basicAuthString =
      "Basic ${base64Encode(utf8.encode("${iservCredentials.item1}:${iservCredentials.item2}"))}";
  final response = await client
      .get(Uri.parse(url), headers: {"authorization": basicAuthString});
  if (response.statusCode != 200) {
    log("Cannot get caldav calendar data with url $url. Error code ${response.statusCode}",
        name: "parsing.calendar");
    return calendarData;
  }
  return parseToCalendarData(response.body, type, calendarData);
...
void executeIcsRecurrenceRule(String rRuleString,
    CalendarDataPoint origDataPoint, CalendarData calendarData) {
  ...
  if (weekStartDate.difference(origDataPoint.startDate).inDays.isNegative) {
    return; // Recurrent rule hasn't started yet.
  }
  ...
  for (final instance in instances.take(5)) {
    final newStartDate = DateTime(
        ...
        instance.month,
        instance.day,
        origDataPoint.startDate.hour,
        origDataPoint.startDate.minute,
        origDataPoint.startDate.second);
...
CalendarData parseToCalendarData(
    String iCalendarString, CalendarType type, CalendarData calendarData) {
  ...
  for (final data in parsedData.reversed) {
    if (data["type"] == "VEVENT") {
      ...
      final dataPoint =
          CalendarDataPoint(type, summary, dateStartDateTime, dateEndDateTime);
      if (rRule != null) {
